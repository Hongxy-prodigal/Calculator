...
//小数点
void Painting::on_btnPeriod_clicked()
{
    //exp不能有小数点. 判断是否存在小数点
...
//正负号
void Painting::on_btnSign_clicked()
{
...
//操作符
void Painting::btnOperatorClicked()
{
...
//操作符入栈
void Painting::pushCode(const QString &tempCode)
{
    qDebug() << codes;
    qDebug() << operands;
    QString result;
...
//判断优先级
int Painting::comparePriority(QString c)
{
...
//等于号
void Painting::on_btnEqual_clicked()
{
    if (equal != 1) {
        QString result = "";
...
//单操作符
//单操作数叠加问题
void Painting::btnUniOperatorClicked()
{
...
//左括号
void Painting::on_btnLeftBracket_clicked()
{
    if (code == "") {        //数字跟括号 直接相乘
        ui->btnMul->click();
    }
...
//右括号
void Painting::on_btnRightBracket_clicked()
{
    //如果没有左括号 就不加右括号
    if (Bracket == 0) {
        return;
...
void Painting::on_btnExp_clicked()
{
...
void Painting::on_btnRand_clicked()
{
    srand(time(nullptr));
    // 生成随机数
...
void Painting::triangleCalculate(double &result, QString &op, QString &temp,
                                 QString &resultString)
{
    //正常的三角函数
    if (op == "sin") {
...
void Painting::functionCalculate(double &result, QString &op, QString &temp,
                                 QString &resultString)
{
    if (op == "⌊x⌋") {
...
void Painting::standardCalculate(double &result, QString &op, QString &temp,
                                 QString &resultString)
{
    if (op == "x²") {
